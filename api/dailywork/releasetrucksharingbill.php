...
$conn = $db->connection();
$ReleaseTruckSharingBill = new ReleaseTruckSharingBillModel();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($data->actionCode) {
        case ActionCode::GET_ALL_RELEASE_TRUCK_SHARING_BILL:
            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::GET_ALL_RELEASE_TRUCK_SHARING_BILL) {
                    echo json_encode(array(
                ...
                $getAllReleaseTruckSharingBill = $ReleaseTruckSharingBill->getAllReleaseTruckSharingBill($conn);
                $num_row = $getAllReleaseTruckSharingBill->rowCount();
                $getAllReleaseTruckSharingBill_arr['data'] = [];
                if ($num_row > 0) {
                    while ($row = $getAllReleaseTruckSharingBill->fetch(PDO::FETCH_ASSOC)) {
                        extract($row);
                        $data = array(
                        ...
                        array_push($getAllReleaseTruckSharingBill_arr['data'], $data);
                    }
                    $res = array(
                        // 'totalResult' => (int)$total_result,
                        'result' => $num_row,
                        'data' => $getAllReleaseTruckSharingBill_arr['data'],
            ...
            break;
        case ActionCode::GET_RELEASE_TRUCK_SHARING_BILL:
            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::GET_RELEASE_TRUCK_SHARING_BILL) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
                ...
                if (!isset($data->billReleaseTruckSharingNo) || empty($data->billReleaseTruckSharingNo)) {
                    ...
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid bill Release Truck Sharing No'
                    ));
                    return;
                }
                ...
                $getReleaseTruckSharingBill = $ReleaseTruckSharingBill->getReleaseTruckSharingBill($conn, $billReleaseTruckSharingNo);
                $num_row = $getReleaseTruckSharingBill->rowCount();
                $getReleaseTruckSharingBill_arr['data'] = [];
                if ($num_row > 0) {
                    while ($row = $getReleaseTruckSharingBill->fetch(PDO::FETCH_ASSOC)) {
                        extract($row);
                        $data = array(
                            "billReleaseTruckSharingNo" => $bill_release_truck_sharing_no,
                            "goDate" => $go_date,
                        ...
                        array_push($getReleaseTruckSharingBill_arr['data'], $data);
                    }
                    $res = array(
                        // 'totalResult' => (int)$total_result,
                        'result' => $num_row,
                        'data' => $getReleaseTruckSharingBill_arr['data'],
                        'responseCode' => ErrorCode::SUCCESS,
                        'message' => 'success',
                        'timestamp' => $timestamp,
                        'actionCode' => $actionCode,
            ...
            break;
        case ActionCode::CREATE_RELEASE_TRUCK_SHARING_BILL:
            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::CREATE_RELEASE_TRUCK_SHARING_BILL) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
        ...
        case ActionCode::UPDATE_RELEASE_TRUCK_IN_BILL:
            try {
                ...
                if (!$check_bill_release_truck_sharing_no) {
                    http_response_code(200);
                    echo json_encode(array(
                        'responseCode' => ErrorCode::UPDATE_RELEASE_TRUCK_SHARING_BILL_FAIL,
                        'message' => 'not found bill release truck sharing no in the system. please use other',
                    ));
                    return;
                }

                $result = $ReleaseTruckSharingBill->updateReleaseTruckSharingBill(
                        $conn, $billReleaseTruckSharingNo, $goDate, $backDate, $licensePlate, $licensePlateBackPart, $driverFirst, $driverSec, $dateShipping, $storeId, $product,
                        // $origin,
            ...
            break;
        case ActionCode::DELETE_RELEASE_TRUCK_SHARING_BILL:
            try {
                if ($data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::DELETE_RELEASE_TRUCK_SHARING_BILL) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
