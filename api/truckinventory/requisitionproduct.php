...
$conn = $db->connection();
$RequisitionProduct = new RequisitionProductModel();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($data->actionCode) {
        case ActionCode::GET_ALL_TIS_REQUISITION_PRODUCT:
            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::GET_ALL_TIS_REQUISITION_PRODUCT) {
                    echo json_encode(array(
                ...
                $get_all_requisition_product = $RequisitionProduct->getAllRequisition($conn, $fromDoumentNo, $toDocumentNo, $fromDate, $toDate);
                $num_row = $get_all_requisition_product->rowCount();
                $get_all_requisition_product_arr['data'] = [];
                if ($num_row > 0) {
                    while ($row = $get_all_requisition_product->fetch(PDO::FETCH_ASSOC)) {
                        extract($row);
                        $data = array(
                            "no" => $no,
                        ...
                        array_push($get_all_requisition_product_arr['data'], $data);
                    }
                    http_response_code(200);
                    echo json_encode(array(
                        // 'totalResult' => (int)$total_result,
                        'result' => $num_row,
                        'data' => $get_all_requisition_product_arr['data'],
            ...
            break;
        case ActionCode::GET_TIS_REQUISITION_PRODUCT:

            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::GET_TIS_REQUISITION_PRODUCT) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
                ...
                $get_requisition_product = $RequisitionProduct->getRequisition($conn, $documentNo);
                $num_row = $get_requisition_product->rowCount();
                $get_requisition_product_arr['data'] = [];
                if ($num_row > 0) {
                    while ($row = $get_requisition_product->fetch(PDO::FETCH_ASSOC)) {
                        extract($row);
                        $data = array(
                            "no" => $no,
                            "documentNo" => $document_no,
                        ...
                        array_push($get_requisition_product_arr['data'], $data);
                    }
                    http_response_code(200);
                    echo json_encode(array(
                        // 'totalResult' => (int)$total_result,
                        'result' => $num_row,
                        'data' => $get_requisition_product_arr['data'],
                        'responseCode' => ErrorCode::SUCCESS,
                        'message' => 'success',
                        'timestamp' => $timestamp,
                        'actionCode' => $actionCode,
        ...
        case ActionCode::CREATE_TIS_REQUISITION_PRODUCT:
            try {

                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::CREATE_TIS_REQUISITION_PRODUCT) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
                ...
                $check_data_exists = $RequisitionProduct->checkRequisitionProduct($conn, $documentNo);
                if ($check_data_exists) {
                    http_response_code(200);
                    echo json_encode(array(
                        'responseCode' => ErrorCode::CREATE_TIS_PRODUCT_FAIL,
                        'message' => 'document no is exist, please use other document no.'
                    ));
                    return;
                }
                ...
                $result_create = $RequisitionProduct->createRequisition(
                    $conn,
                    $documentNo,
                    $documentDate,
                    $note,
                ...
                http_response_code(200);
                echo json_encode(array(
                    'responseCode' => ErrorCode::CREATE_TIS_REQUISITION_PRODUCT_FAIL,
                    'message' => 'fail',
                    'timestamp' => $timestamp,
                    'actionCode' => $actionCode,
                    'actionNodeId' => $actionNodeId
            ...
            break;
        case ActionCode::UPDATE_TIS_REQUISITION_PRODUCT:
            try {
                if (!isset($data->actionCode) || $data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::UPDATE_TIS_REQUISITION_PRODUCT) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
                ...
                if (!$check_data_exists) {
                    http_response_code(200);
                    echo json_encode(array(
                        'responseCode' => ErrorCode::NOT_FOUND_TIS_REQUISITION_PRODUCT,
                        'message' => 'not found'
                    ));
                    return;
                }
                ...
                $conn->beginTransaction();
                $result_update = $RequisitionProduct->udpateRequisition(
                    $conn,
                    $documentNo,
                    $documentDate,
                    $note,
            ...
            break;
        case ActionCode::DELETE_TIS_REQUISITION_PRODUCT:
            try {
                if ($data->actionCode == null || $data->actionCode == "" || $data->actionCode != ActionCode::DELETE_TIS_REQUISITION_PRODUCT) {
                    echo json_encode(array(
                        'responseCode' => ErrorCode::INVALID_DATA_SEND,
                        'message' => 'Invalid Action Code'
                    ));
                ...
                $conn->beginTransaction();
                $delete_result = $RequisitionProduct->deleteRequisition($conn, $documentNo);
                if ($delete_result) {
                    http_response_code(200);
                    echo json_encode(array(
                        'responseCode' => ErrorCode::SUCCESS,
